...
<style>
  ...
  *{box-sizing:border-box}
  body{margin:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:linear-gradient(180deg,#0b1020,#0f1730);color:var(--text)}
  .wrap{max-width:1280px;margin:0 auto;padding:24px}
  .hero{padding:20px 24px;border:1px solid var(--line);border-radius:18px;background:linear-gradient(135deg,rgba(0,212,255,.08),rgba(206,147,216,.08))}
  ...
  .section{margin-top:18px;padding:18px;background:var(--panel);border:1px solid var(--line);border-radius:18px}
  .section h2{margin:0 0 14px;font-size:18px}
  .subgrid{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:14px}
  .class-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px}
  .class-card{background:var(--panel-2);border:1px solid var(--line);border-radius:14px;padding:14px}
  .cc-label{font-weight:700;font-size:18px;margin-bottom:12px}
  .cc-metric{display:flex;justify-content:space-between;gap:10px;padding:6px 0;border-top:1px solid rgba(255,255,255,.04)}
  .cc-metric:first-of-type{border-top:none;padding-top:0}
  .cc-name{color:var(--muted);font-size:12px}
  .cc-val{font-weight:700}
  .feat-bar,.sbar{margin-bottom:12px}
  .feat-head,.sbar-head{display:flex;justify-content:space-between;gap:12px;margin-bottom:6px}
  .feat-name,.sbar-name{font-weight:600}
  .feat-val,.sbar-info{color:var(--muted);font-size:12px}
  .feat-track,.sbar-track{height:10px;border-radius:999px;background:rgba(255,255,255,.06);overflow:hidden}
  .feat-fill,.sbar-fill{height:100%;border-radius:999px}
  table{width:100%;border-collapse:collapse}
  th,td{padding:10px 8px;border-bottom:1px solid rgba(255,255,255,.06);text-align:left;font-size:14px}
  th{color:var(--muted);font-weight:600}
  .t-table{display:grid;gap:8px}
  .t-head,.t-row{display:grid;grid-template-columns:56px repeat(9,minmax(0,1fr)) 110px;gap:8px;align-items:center}
  .t-head{color:var(--muted);font-size:12px;text-transform:uppercase;letter-spacing:.08em}
  .t-row{padding:10px 12px;border:1px solid rgba(255,255,255,.05);border-radius:12px;background:var(--panel-2)}
  .muted{color:var(--muted)}
  .win{color:var(--g);font-weight:700}
  .lose{color:var(--r);font-weight:700}
  .to-c{color:var(--o);font-weight:700}
  .long{color:var(--a);font-weight:700}
  .short{color:#ff7043;font-weight:700}
  .footer-note{margin-top:14px;color:var(--muted);font-size:12px}
  pre{margin:0;white-space:pre-wrap;word-break:break-word;color:#cfe6ff}
</style>
"""


def _training_html_template(**ctx) -> str:
...
<body>
  <div class="wrap">
    <div class="hero">
      ...
      <p>Validation accuracy {ctx['val_acc']*100:.2f}% | Train-validation gap {ctx['gap']*100:.2f}%</p>
    </div>

    <div class="grid">
      <div class="card"><div class="label">Samples</div><div class="value">{ctx['n_samples']:,}</div></div>
      <div class="card"><div class="label">Train Rows</div><div class="value">{ctx['n_train']:,}</div></div>
      <div class="card"><div class="label">Val Rows</div><div class="value">{ctx['n_val']:,}</div></div>
      <div class="card"><div class="label">Features</div><div class="value">{ctx['n_features']}</div></div>
      <div class="card"><div class="label">Train Accuracy</div><div class="value">{ctx['train_acc']*100:.2f}%</div></div>
      <div class="card"><div class="label">Val Accuracy</div><div class="value">{ctx['val_acc']*100:.2f}%</div></div>
      <div class="card"><div class="label">WF Accuracy</div><div class="value">{ctx['avg_wf']*100:.2f}%</div></div>
      <div class="card"><div class="label">WF Sharpe</div><div class="value">{ctx['avg_sh']:.3f}</div></div>
    </div>

    <div class="section">
      <h2>Class Metrics</h2>
      <div class="class-grid">{ctx['class_html']}</div>
    </div>

    <div class="section">
      <h2>Top Features</h2>
      <div>{ctx['feat_bars_html']}</div>
    </div>
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Train Size</th><th>Test Size</th><th>Accuracy</th><th>Sharpe</th><th>Status</th>
          </tr>
        </thead>
        <tbody>{ctx['wf_rows_html']}</tbody>
      </table>
      <div class="footer-note">Embedded metrics payload available below for debugging or downstream parsing.</div>
      <pre>{ctx['chart_data']}</pre>
    </div>
  </div>
</body>
</html>"""
...
def _backtest_html_template(**ctx) -> str:
    equity_delta = _safe_float(ctx['equity_end']) - _safe_float(ctx['equity_start'])
    equity_class = 'win' if equity_delta >= 0 else 'lose'
    return f"""<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
<body>
  <div class="wrap">
    ...
    <div class="grid">
      <div class="card"><div class="label">Trades</div><div class="value">{ctx['T']}</div></div>
      <div class="card"><div class="label">Win Rate</div><div class="value">{ctx['wr']*100:.2f}%</div></div>
      <div class="card"><div class="label">PnL</div><div class="value">{ctx['pnl']:+.2f}</div></div>
      <div class="card"><div class="label">Sharpe</div><div class="value">{ctx['sh']:.3f}</div></div>
      <div class="card"><div class="label">Max Drawdown</div><div class="value">{ctx['mdd']:.2f}</div></div>
      <div class="card"><div class="label">Avg TP / SL</div><div class="value">{ctx['tp_avg']:.1f} / {ctx['sl_avg']:.1f}</div></div>
      <div class="card"><div class="label">Avg Duration</div><div class="value">{ctx['dur_avg']:.1f}m</div></div>
      <div class="card"><div class="label">Dominant Style</div><div class="value">{ctx['dominant']}</div></div>
      <div class="card"><div class="label">Equity Change</div><div class="value {equity_class}">{equity_delta:+.2f}</div></div>
      <div class="card"><div class="label">Bars Evaluated</div><div class="value">{_safe_int(ctx['n_test_bars'])}</div></div>
    </div>

    <div class="section">
      <h2>Style Distribution</h2>
      <div>{ctx['style_html']}</div>
    </div>
    ...
    <div class="section">
      ...
      <div class="t-table">
        <div class="t-head">
          <div>#</div><div>Dir</div><div>Entry</div><div>Exit</div><div>TP</div><div>SL</div>
          <div>Pips</div><div>PnL</div><div>Duration</div><div>Result</div><div>State</div>
        </div>
        {ctx['trade_rows_html']}
      </div>
    </div>
